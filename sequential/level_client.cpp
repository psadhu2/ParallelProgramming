...
std::vector<std::vector<std::string>> bfs(const std::string& start, int depth) {
  ...
  for (int d = 0;  d < depth; d++) {
    ...
    int num_threads = std::min(MAX_THREADS, (int)levels[d].size());

    for (int t = 0; t < num_threads; t++) {
      int start_idx = (t * levels[d].size()) / num_threads;
      int end_idx = ((t + 1) * levels[d].size()) / num_threads;

      threadgroup.emplace_back([&, start_idx, end_idx]() {
        CURL * local_curl = curl_easy_init();
        for (int i = start_idx; i < end_idx; i++) {
          for (const auto& neighbor : get_neighbors(fetch_neighbors(local_curl, levels[d][i]))) {
...
int main(int argc, char* argv[]) {
    ...
    // curl handles are not thread safe so created local handles inside the bfs function so each thread can have its seperate handle
